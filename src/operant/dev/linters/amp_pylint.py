...
class _Pylint(liaction.Action):
    ...
    def _execute(self, file_name: str, pedantic: int) -> List[str]:
        ...
        if pedantic < 2:
            ...
            ignore.extend(
                [
                    ...
                    # `no-member` errors.
                    "E0401",
                    # Repo-specific: typed, Protocol-heavy, pytest codebase.
                    # mypy runs in the same pipeline and is the type
                    # authority, so pylint's member/return inference (weak on
                    # dynamic modules, Optional short-circuits, and Protocol
                    # return types) only adds false positives here.
                    "E1101",  # no-member (alembic.op/context, union narrow)
                    "E1111",  # assignment-from-no-return (Protocol returns)
                    "E1135",  # unsupported-membership-test (Optional guard)
                    "W0613",  # unused-argument (Protocol/callback/fake sigs)
                    "W2301",  # unnecessary-ellipsis (Protocol `...` bodies)
                    "W0718",  # broad-exception-caught (isolation points)
                    "W0622",  # redefined-builtin (`id`, `input` params)
                    "C1803",  # use-implicit-booleaness (style)
                    "R0911",  # too-many-return-statements (sibling of R0912)
                    "R0917",  # too-many-positional-arguments (like R0913)
                    "R1711",  # useless-return
                    "R1714",  # consider-using-in (style)
                    "R1732",  # consider-using-with (deliberate context mgmt)
                    "W0102",  # dangerous-default (typer Option list defaults)
                    "W0212",  # protected-access (within-module helpers)
                ]
            )
            # Unit test.
            if is_test_code_tmp:
                ignore.extend(
                    [
                        ...
                        "W0212",
                        # pytest fixtures reuse the fixture name as the
                        # parameter name by design.
                        "W0621",  # redefined-outer-name
                        # round-trip tests assert on the exact type.
        ...
        if pedantic < 1:
            ignore.extend(
                [
                    ...
                    "C0111",
                    # black owns code line length (82); C0301 at 100 then only
                    # fires on long string literals (e.g. prompt templates)
                    # that black cannot wrap.
                    "C0301",
                ]
            )
        if ignore:
        ...
        opts.append("--ignored-modules=pandas,alembic.op,alembic.context")
