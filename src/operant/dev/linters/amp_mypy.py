...
class _Mypy(liaction.Action):
    ...
    def _config_path() -> str:
        ...
        """
        # Pass an explicit starting directory. BSD find, unlike GNU find, does
        # not default to the current directory and rejects the command without
        # one, which makes the unqualified form fail on macOS.
        cmd = "find . -name 'mypy.ini'"
        _, path = hsystem.system_to_string(cmd)
        # Pick the one closer to the current dir, i.e. the shallowest match,
        # so a config at the repo root wins over one nested in a subdirectory.
        paths = sorted(path.split("\n"), key=lambda p: (p.count(os.sep), p))
        path = paths[0]
