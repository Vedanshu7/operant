...
def _check_import(file_name: str, line_num: int, line: str) -> str:
    ...
    if m:
        # `typing` is exempt upstream; `__future__` is exempt here so
        # `from __future__ import annotations` stays allowed (keeps
        # annotations lazy: no eager-eval forward-ref or module/field
        # name collisions across the codebase).
        if m.group(1) not in ("typing", "__future__"):
            msg = (
